...
Various steps / good practices to execute after installation > classic process review in order to prepare for automation.

LATER: Make changes in a custom DockerFile ? Ansible tasks ? Or populate volumes from template ?

Make difference between configuration and defaults. Defaults will be treated in the next folder.
...
- Plugins > Installed plugins > (List) > Delete
